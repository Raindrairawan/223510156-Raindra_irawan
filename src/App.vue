<template>
  <div id="app-container">
    ...
    <div class="content">
      ...
      <slot></slot> <!-- Menambahkan slot di sini -->
    </div>
  </div>
</template>
...
<script>
  export default {
    props: {
      user: {
        type: Object,
        default: () => ({ name: 'Guest' }) // Menetapkan nilai default
      }
    },
    data() {
      return {
    ...
    methods: {
      ...
      updateTodo(todo) {
        ...
        if (index !== -1) {
          this.todos[index].completed = !this.todos[index].completed; // Toggle status completed
        }
      }
    }
  }
